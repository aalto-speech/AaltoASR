...
int HypoStack::find_similar(const Hypo &hypo, int words)
{
...
void HypoStack::sorted_insert(const Hypo &hypo)
{
...
Search::Search(Expander &expander, const Vocabulary &vocabulary) :
			...
			// Options
			m_end_frame(-1), m_lm_scale(1), m_lm_offset(0), m_unk_offset(0),
			m_verbose(0), m_print_probs(false), m_multiple_endings(0),
			m_print_indices(false), m_print_frames(false),
			m_last_printed_path(NULL),

			// Pruning options
			m_word_boundary(-1), m_dummy_word_boundaries(true),
			m_word_limit(0), m_word_beam(1e10), m_prune_similar(0),
			m_hypo_limit(0), m_hypo_beam(1e10), m_global_beam(1e10)
{
}
...
void Search::set_word_boundary(const std::string &word)
{
	...
	if (m_word_boundary == 0) {
		fprintf(stderr,
				"Search::set_word_boundary(): word boundary not in vocabulary\n");
		exit(1);
	}
}

void Search::print_prunings()
{
	printf("stack expansions: %d\n"
		"hypo insertions:  %d\n"
		"limit prunings:   %d\n"
		"similar prunings: %d\n", m_stack_expansions, m_hypo_insertions,
			m_limit_prunings, m_similar_prunings);
}

void Search::print_path(HypoPath *path)
{
...
void Search::print_hypo(const Hypo &hypo)
{
...
void Search::reset_search(int start_frame)
{
...
int Search::frame2stack(int frame) const
{
...
void Search::ensure_stack(int frame)
{
...
float Search::compute_lm_log_prob(const Hypo &hypo)
{
	float lm_log_prob = 0;
		...
		for (int lm = 0; lm < m_ngrams.size(); lm++) {
			...
			for (int j = m_history_lex.size(); j > 0; j--) {
				if (m_history_lm.size() == m_ngrams[lm].ngram->order())
					break;
				m_history_lm.push_front(
						m_ngrams[lm].lex2lm[m_history_lex[j - 1]]);
			}
...
void Search::update_global_pruning(int frame, float log_prob)
{
...
void Search::insert_hypo(int target_frame, const Hypo &hypo)
{
...
void Search::expand_hypo_with_word(const Hypo &hypo, int word,
		int target_frame, float ac_log_prob)
{
	...
	if (m_dummy_word_boundaries && m_word_boundary > 0 && word
			!= m_word_boundary) {
...
void Search::expand_hypo(const Hypo &hypo)
{
	...
	for (int w = 0; w < words.size(); w++) {
		...
		if (words[w]->best_avg_log_prob < words[0]->best_avg_log_prob
				* m_word_beam)
			continue;
		...
		if (m_multiple_endings > 0) {
			for (int i = -m_multiple_endings; i <= m_multiple_endings; i++) {
				...
				if (length < words[w]->first_length || length
						>= words[w]->last_length)
					continue;
				...
				expand_hypo_with_word(hypo, words[w]->word_id, hypo.frame
						+ length, words[w]->log_probs[length]);
			}
		}
		...
		// Expand only to the best ending time.
			...
			expand_hypo_with_word(hypo, words[w]->word_id, hypo.frame
					+ words[w]->best_length, words[w]->best_log_prob());
		}
	}
}

void Search::find_best_words(int frame)
{
...
bool Search::expand_stack(int frame)
{
...
void Search::expand_words(int frame, const std::string &words)
{
	...
	while (in >> str) {
		...
		expand_hypo_with_word(stack(frame)[0], word->word_id, frame
				+ word->best_length, word->best_log_prob());
		frame += word->best_length;
		fprintf(stderr, "%d\t%s\n", frame, str.c_str());
	}
}

void Search::go(int frame)
{
	while (m_frame < frame) {
		HypoStack &stack = this->stack(frame);
		stack.clear();
...
bool Search::recognize_segment(int start_frame, int end_frame)
{
