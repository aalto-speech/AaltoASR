...
main(int argc, char *argv[])
{
  ...
  try {
    config("usage: pca [OPTION...]\n")
      ...
      ('M', "module=NAME", "arg", "", "pca module name")
	 ...
	 recipe_index++) 
    {
      ...
      for (int f = start_frame; f < end_frame; f++)
      {
        ...
        const FeatureVec vec = (pca_mod_src == NULL? temp_fvec :
                                pca_mod_src->at(f));
        for (int d = 0; d < dim; d++)
        {
    ...
    for (int d = 0; d < dim; d++)
    {
      ...
      if (std::fabs(mean[d]) > 0.000001)
        std::cout << "Warning, mean for dimension "
                  << d << " is " << mean[d] << " and pca assumes zero!"
                  << std::endl;
    }
    scale.resize(dim);
    for (int d = 0; d < dim; d++)
    ...
    if (config["write-config"].specified)
      gen.write_configuration(io::Stream(config["write-config"].get_str(),"w"));
  }
