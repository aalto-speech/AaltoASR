...
void Pcgmm::map_m2v(const LaGenMatDouble &m, LaVectorDouble &v)
{
...
void Pcgmm::map_v2m(const LaVectorDouble &v, LaGenMatDouble &m)
{
...
int Pcgmm::initialize_basis_svd(const std::vector<int> &c,
...
int Pcgmm::reset_basis(const unsigned int basis_dim, 
...
void Pcgmm::calculate_precision(const std::vector<double> &lambda,
...
void Pcgmm::calculate_covariance(const std::vector<double> &lambda,
...
void Pcgmm::gradient(const std::vector<double> &lambda,
...
void Pcgmm::polak_ribiere_direction(const LaVectorDouble &old_grad,
...
double Pcgmm::line_search_more_thuente(const LaGenMatDouble &P,
...
double Pcgmm::eval_aux_function(const LaVectorDouble &eigs,
				double step,
				double trace)
{
  double f=0;
...
double Pcgmm::eval_aux_function_derivative(const LaVectorDouble &eigs,
					   double step,
					   double trace)
{
  double d=trace;
...
void Pcgmm::limit_line_search(const LaGenMatDouble &R,
...
void Pcgmm::generalized_eigenvalues(const LaGenMatDouble &A,
...
void Pcgmm::cholesky_factor(const LaGenMatDouble &A, LaGenMatDouble &B)
{
  assert(A.rows() == A.cols());
...
bool Pcgmm::is_spd(const LaGenMatDouble &A)
{
  assert(A.rows()==A.cols());
