...
void collect_dur_stats(aku::PhnReader *phn_reader, int skip_states)
{
	...
	while (phn_reader->next_phn_line(phn)) {
		if (phn.state == -1)
			throw std::string(
					"Collecting duration statistics requires phn files with state numbers!");

		add_duration(phn.label[0], phn.state, phn.end - phn.start, skip_states);
	}
}

void write_dur_histograms(const std::string &filename, int skip_states)
{
...
double negative_gamma_loglikelihood(double a, double mean_log, double log_mean)
{
	return a * (1 + log_mean - log(a)) + lgamma(a) + (1 - a) * mean_log;
}

bool estimate_gamma_models(std::vector<int> &hist, double &a_out, double &b_out)
{
	...
	if (count < 2) {
		if (info > 0)
			fprintf(stderr,
					"Warning: Needs at least 2 points for estimating gamma models\n");
		return false;
	}
	mean = 0;
...
void write_gamma_models(const std::string &filename, int skip_states,
		int min_count)
{
	io::Stream out_file(filename, "w");
	fprintf(out_file, "4\n%d\n", model.num_states());
	for (int i = 0; i < (int) dur_table.size(); i++) {
		double a, b;
		if (i < skip_states || std::accumulate(dur_table[i].begin(),
				dur_table[i].end(), 0) < min_count || !estimate_gamma_models(
				dur_table[i], a, b)) {
			if (info > 0)
...
int main(int argc, char *argv[])
{
	...
	try {
		config("usage: dur_est [OPTION...]\n")('h', "help", "", "",
				"display help")('p', "ph=FILE", "arg must", "",
				"HMM definitions")('r', "recipe=FILE", "arg must", "",
				"recipe file")('O', "ophn", "", "", "use output phns")('M',
				"maxdur=INT", "arg", "100",
				"maximum duration noted (default 100)")('\0', "hist=FILE",
				"arg", "", "write duration histograms to file")('\0',
				"gamma=FILE", "arg", "",
				"write gamma models for durations to file")('\0', "skip=INT",
				"arg", "0", "skip first INT states")('\0', "mincount=INT",
				"arg", "10", "minimum occurance count to estimate gamma models")(
		...
		for (int f = 0; f < (int) recipe.infos.size(); f++) {
			...
			recipe.infos[f].init_phn_files(NULL, false, false,
					config["ophn"].specified, NULL, &phn_reader);
