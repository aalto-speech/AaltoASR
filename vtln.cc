		...
		while (seg->next_frame()) {
			...
			for (i = 0; i < (int) pdfs.size(); i++) {
				...
				speaker_stats[cur_speaker].log_likelihoods[cur_warp_index]
						+= util::safe_log(
								pdfs[i].prob * model.pdf_likelihood(
										pdfs[i].index, fea_vec));
			}
		}
	}
}
...
int main(int argc, char *argv[])
{
	...
	try {
		config("usage: vtln [OPTION...]\n")('h', "help", "", "", "display help")(
				...
				"Gaussian kernels")('m', "mc=FILE", "arg", "",
				"kernel indices for states")('p', "ph=FILE", "arg", "",
				"HMM definitions")('c', "config=FILE", "arg must", "",
				"feature configuration")('r', "recipe=FILE", "arg must", "",
				"recipe file")('O', "ophn", "", "", "use output phns for VTLN")(
				'v', "vtln=MODULE", "arg must", "", "VTLN module name")('S',
				"speakers=FILE", "arg must", "",
				"speaker configuration input file")('o', "out=FILE", "arg", "",
				"output speaker configuration file")('s', "savesum=FILE",
				"arg", "", "save summary information (loglikelihoods)")('\0',
				"snl", "", "", "phn-files with state number labels")('\0',
				"rsamp", "", "",
				"phn sample numbers are relative to start time")('\0',
				"grid-size=INT", "arg", "21",
				"warping grid size (default: 21/5)")('\0', "grid-rad=FLOAT",
				"arg", "0.1", "radius of warping grid (default: 0.1/0.03)")(
				'\0', "relative", "", "",
				"relative warping grid (and smaller grid defaults)")('B',
				"batch=INT", "arg", "0",
