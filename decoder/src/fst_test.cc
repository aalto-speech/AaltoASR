...
void test_fstsearch(std::string fst_fname, std::string lna_name, std::string hmm_name, std::string dur_name) {
  ...
  fsts.set_beam(1000.0f);
  fsts.set_token_limit(5000);
  ...
  float token_dist(fsts.token_confidence());
  fprintf(stderr, "%s %.4f\n", result.c_str(), token_dist);
}
...
int main(int argc, char *argv[]) {
  ...
  //std::string lna_name("A11344Q1.lna");
  //std::string lna_name("A12486S3.lna");
  std::string lna_name("A12010M1.lna");
